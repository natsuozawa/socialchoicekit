...
class MatchTwoQueries:
  """
  Match-TwoQueries (Amanatidis et al. 2022) achieves a distortion of O(root n) with two queries asked per agent. The first query is the agent's favorite item. The second query is the agent's cardinal value of the item they are assigned to in a 'sufficiently representative assignment' generated by the root n serial dictatorship routine.
